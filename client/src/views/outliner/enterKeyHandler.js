...
export function handleEnterKey(event, editor, now, logCursorTiming, pushDebug, pendingEmptyCaretRef) {
  ...
  if (onlyParagraph && paragraphEmpty && isAtStart && isAtEnd) {
    ...
    // Ensure caret ends up in the newly inserted (empty) sibling without referencing
    // undefined variables like splitMeta/selectionAdjusted.
    ...
    const enforceEmptyCaret = () => {
      try {
        ...
        if (newIndex >= parentNode.childCount) return
